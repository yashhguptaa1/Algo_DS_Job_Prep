package GRAPHS.L200_NumberOfIslands;
...
public class F_NumberOfIslands {
    ...
    public int numIslandsV2(char[][] adjGraph) {
        ...
        for (int i = 0; i < adjGraph.length; i++) {
            for (int j = 0; j < adjGraph[0].length; j++) {
                ...
                if (adjGraph[i][j] == '1') {
                    ...
                    // correct answer even if we place this after dfs
                    // countConnectedComponents++;
 ...
 * Add to List
 * 
 * Share Given an m x n 2D binary grid grid which represents a map of '1's
