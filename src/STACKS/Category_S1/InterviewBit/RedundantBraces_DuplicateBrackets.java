...
public class RedundantBraces_DuplicateBrackets {
    public static int braces(String A) {
        ...
        for (int i = 0; i < A.length(); i++) {
            ...
            // WE want to do comparison only when our current state becomes ) a closing
            // bracket
            // otherwise go on saving all previous state to be accessed later
            if (currState != ')') {
                st.push(currState);
            } else {
 ...
 * 
 * Return 1 if string has redundant braces, else return 0. For Example
 * 
 * Input 1: A = "((a + b))" A=(a) Output 1: 1 Explanation 1: ((a + b)) has
 * redundant braces so answer will be 1. (a) has redundant braces Input 2: A =
 * "(a + (a + b))" Output 2: 0 Explanation 2: (a + (a + b)) doesn't have have
 * any redundant braces so answer will be 0.
 */